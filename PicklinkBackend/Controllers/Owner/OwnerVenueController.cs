...
public class OwnerVenueController : ControllerBase
{
    ...
    private ActionResult<T> ToActionResult<T>(OwnerVenueServiceResult<T> result) =>
        result.Status switch
        {
            OwnerVenueServiceResultStatus.Success => Ok(result.Value),
            OwnerVenueServiceResultStatus.Created => CreatedAtAction(result.CreatedActionName, result.CreatedRouteValues, result.Value),
            OwnerVenueServiceResultStatus.NoContent => NoContent(),
            OwnerVenueServiceResultStatus.BadRequest => BadRequest(result.Error),
            OwnerVenueServiceResultStatus.Unauthorized => result.Error is null ? Unauthorized() : Unauthorized(result.Error),
            OwnerVenueServiceResultStatus.Forbidden => result.Error is null ? Forbid() : StatusCode(StatusCodes.Status403Forbidden, result.Error),
            OwnerVenueServiceResultStatus.NotFound => result.Error is null ? NotFound() : NotFound(result.Error),
            OwnerVenueServiceResultStatus.Conflict => Conflict(result.Error),
            OwnerVenueServiceResultStatus.StatusCode => StatusCode(result.RawStatusCode ?? StatusCodes.Status500InternalServerError, result.Value ?? result.Error),
            _ => StatusCode(StatusCodes.Status500InternalServerError, result.Error)
        };

    private IActionResult ToActionResult(OwnerVenueServiceResult result) =>
        result.Status switch
        {
            OwnerVenueServiceResultStatus.Success => result.Value is null ? Ok() : Ok(result.Value),
            OwnerVenueServiceResultStatus.Created => CreatedAtAction(result.CreatedActionName, result.CreatedRouteValues, result.Value),
            OwnerVenueServiceResultStatus.NoContent => NoContent(),
            OwnerVenueServiceResultStatus.BadRequest => BadRequest(result.Error),
            OwnerVenueServiceResultStatus.Unauthorized => result.Error is null ? Unauthorized() : Unauthorized(result.Error),
