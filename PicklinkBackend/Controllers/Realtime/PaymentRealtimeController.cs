...
[ApiController]
[AllowAnonymous]
[Route("api/realtime/payments")]
public sealed class PaymentRealtimeController : ControllerBase
{
    private readonly PaymentRealtimeNotifier _notifier;

    public PaymentRealtimeController(PaymentRealtimeNotifier notifier)
    {
        _notifier = notifier;
    }
    ...
    public async Task Stream(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                var heartbeat = Task.Delay(TimeSpan.FromSeconds(10), cancellationToken);
                ...
                if (completed == waitForEvent)
                {
                    ...
                    while (subscription.Reader.TryRead(out var notification))
                    {
                        ...
                        await Response.WriteAsync($"event: payment-updated\ndata: {json}\n\n", cancellationToken);
                    }
