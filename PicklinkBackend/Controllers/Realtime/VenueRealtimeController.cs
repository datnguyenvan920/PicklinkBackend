...
[ApiController]
[AllowAnonymous]
[Route("api/realtime/venues")]
public sealed class VenueRealtimeController : ControllerBase
{
    private readonly VenueRealtimeNotifier _notifier;

    public VenueRealtimeController(VenueRealtimeNotifier notifier) => _notifier = notifier;
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                if (completed == waitForEvent)
                {
                    ...
                    while (subscription.Reader.TryRead(out var notification))
                    {
                        ...
                        await Response.WriteAsync($"event: venue-updated\ndata: {json}\n\n", cancellationToken);
                    }
