...
public class PlayerBookingController : ControllerBase
{
    ...
    private ActionResult<T> ToActionResult<T>(PlayerBookingServiceResult<T> result) =>
        result.Status switch
        {
            PlayerBookingServiceResultStatus.Success => Ok(result.Value),
            PlayerBookingServiceResultStatus.NoContent => NoContent(),
            PlayerBookingServiceResultStatus.BadRequest => BadRequest(result.Error),
            PlayerBookingServiceResultStatus.Unauthorized => result.Error is null ? Unauthorized() : Unauthorized(result.Error),
            PlayerBookingServiceResultStatus.Forbidden => result.Error is null ? Forbid() : StatusCode(StatusCodes.Status403Forbidden, result.Error),
            PlayerBookingServiceResultStatus.NotFound => result.Error is null ? NotFound() : NotFound(result.Error),
            PlayerBookingServiceResultStatus.Conflict => Conflict(result.Error),
            PlayerBookingServiceResultStatus.StatusCode => StatusCode(result.RawStatusCode ?? StatusCodes.Status500InternalServerError, result.Value ?? result.Error),
            _ => StatusCode(StatusCodes.Status500InternalServerError, result.Error)
        };

    private IActionResult ToActionResult(PlayerBookingServiceResult result) =>
        result.Status switch
        {
            PlayerBookingServiceResultStatus.Success => result.Value is null ? Ok() : Ok(result.Value),
            PlayerBookingServiceResultStatus.NoContent => NoContent(),
            PlayerBookingServiceResultStatus.BadRequest => BadRequest(result.Error),
            PlayerBookingServiceResultStatus.Unauthorized => result.Error is null ? Unauthorized() : Unauthorized(result.Error),
            PlayerBookingServiceResultStatus.Forbidden => result.Error is null ? Forbid() : StatusCode(StatusCodes.Status403Forbidden, result.Error),
