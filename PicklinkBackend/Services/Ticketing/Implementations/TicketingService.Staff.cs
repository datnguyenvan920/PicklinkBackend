...
public sealed partial class TicketingService
{
        ...
        int? userId, DateOnly date, int page, int pageSize, CancellationToken cancellationToken)
    {
        ...
        var query = _paymentRepository.TicketSessions.Where(item =>
    ...
    {
        if (userId is null) return Unauthorized();
        var session = await SessionGraph(_paymentRepository.TicketSessions.AsNoTracking())
        ...
        CancellationToken cancellationToken)
    {
        ...
        return result.Status == ServiceResultStatus.Success
            ? Ok(MapTicket(result.Value!, DateTime.UtcNow, includeSession: false))
        ...
        CancellationToken cancellationToken)
    {
        ...
        var identityQuery = _paymentRepository.SessionTickets.AsNoTracking()
        ...
        if (!await SqlServerBookingLock.AcquireAsync(
                transaction, $"ticket-session:{ticketIdentity.TicketSessionId}", cancellationToken))
            return Conflict(new { message = "Buổi xé vé đang được cập nhật. Vui lòng thử lại." });
        if (!await SqlServerBookingLock.AcquireAsync(
                transaction, $"ticket-checkin:{ticketIdentity.SessionTicketId}", cancellationToken))
        ...
        await _paymentRepository.AddAuditLogAsync(NewAudit(ticket.TicketSession.Booking.Court.VenueId, userId.Value,
            $"TicketCheckedIn:{ticket.TicketCode}"), cancellationToken);
