...
public partial class CommunityService
{
    ...
    {
        var player = await _communityRepository.Players
        ...
        await _communityRepository.AddPlayerAsync(player, cancellationToken);
    ...
    private async Task<GroupMember?> GetMembershipAsync(
        ...
        CancellationToken cancellationToken)
    {
        return await _communityRepository.GroupMembers
    ...
    private async Task<bool> CanInteractWithGroupAsync(
        ...
        CancellationToken cancellationToken)
    {
        return await _communityRepository.GroupMembers.AnyAsync(
            member =>
    ...
    private async Task<Conversation> EnsureGroupConversationAsync(
        ...
        CancellationToken cancellationToken)
    {
        var conversation = await _communityRepository.Conversations
            .Where(conversation =>
        ...
        {
            var groupName = await _communityRepository.SocialGroups
                .Where(group => group.GroupId == groupId)
        ...
        var acceptedMemberIds = await _communityRepository.GroupMembers
            .Where(member => member.GroupId == groupId && member.Status == AcceptedStatus)
            .Select(member => member.UserId)
            .ToListAsync(cancellationToken);

        var participantIds = await _communityRepository.ConversationParticipants
    ...
    private async Task NotifyGroupManagersAsync(
        ...
        CancellationToken cancellationToken)
    {
        var managerUserIds = await _communityRepository.GroupMembers
            .Where(member =>
                member.GroupId == groupId &&
                member.Status == AcceptedStatus &&
    ...
    private async Task NotifyGroupMembersAsync(
        ...
        CancellationToken cancellationToken)
    {
        var memberUserIds = await _communityRepository.GroupMembers
            .Where(member =>
                member.GroupId == groupId &&
                member.Status == AcceptedStatus &&
