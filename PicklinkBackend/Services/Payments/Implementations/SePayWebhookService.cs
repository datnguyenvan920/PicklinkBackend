...
public sealed class SePayWebhookService
{
    ...
    public async Task<SePayWebhookResult> Process(SePayWebhookRequest request, CancellationToken cancellationToken)
    {
        ...
        var candidate = await _paymentRepository.Payments.AsNoTracking()
            .Where(item => item.BankAccountNumber == request.AccountNumber.Trim()
                ...
                && (item.Status == "Pending" || item.Status == "WaitingForConfirmation")
        ...
        var candidateTicketSessionIds = await _paymentRepository.SessionTickets.AsNoTracking()
        ...
        if (!await SqlServerBookingLock.AcquireAsync(transaction, $"sepay-transaction:{request.Id}", cancellationToken))
            return Fail(409, "Transaction is being processed.");
        if (await _paymentRepository.SePayTransactions.AnyAsync(
                item => item.ExternalTransactionId == request.Id, cancellationToken))
            return Success();
        ...
        if (!await SqlServerBookingLock.AcquireAsync(transaction, $"payment-review:{candidate.PaymentId}", cancellationToken))
        ...
        var bookings = await _paymentRepository.Bookings
            ...
            .ToListAsync(cancellationToken);
        await _paymentRepository.Payments.Where(item => bookingIds.Contains(item.BookingId)).LoadAsync(cancellationToken);

        var paymentIds = groupPayments.Select(item => item.PaymentId).ToArray();
        var sessionTickets = await _paymentRepository.SessionTickets
        ...
        if (matchIds.Length > 0)
            await _paymentRepository.Matches.Include(item => item.MatchParticipants)
                .Where(item => matchIds.Contains(item.MatchId)).LoadAsync(cancellationToken);
        await _paymentRepository.BookingSlots.Include(item => item.Court)
        ...
        {
            await _paymentRepository.AddSePayTransactionAsync(NewSePayTransaction(
                request, payment.PaymentId, "AdditionalRefundPending", utcNow), cancellationToken);
            await NotifyAdditionalTicketTransferAsync(
                payment,
                sessionTickets[0],
        ...
        {
            await _paymentRepository.AddSePayTransactionAsync(NewSePayTransaction(
                request, payment.PaymentId, "ReviewRequired", utcNow), cancellationToken);
            await NotifyPaymentReviewRequiredAsync(
                payment,
                request.TransferAmount,
                reference,
        ...
        if (expectedAmount != request.TransferAmount)
        {
            ...
            await _paymentRepository.AddSePayTransactionAsync(NewSePayTransaction(
                request, payment.PaymentId, ledgerStatus, utcNow), cancellationToken);
        ...
        {
            await _paymentRepository.AddSePayTransactionAsync(NewSePayTransaction(
                request, payment.PaymentId, "TicketRefundPending", utcNow), cancellationToken);
            foreach (var ticket in lateSettlementTickets)
                await MarkLateTicketSettlementAsync(paymentsById[ticket.PaymentId], ticket, utcNow, reference, cancellationToken);
            await _paymentRepository.SaveChangesAsync(cancellationToken);
            await transaction.CommitAsync(cancellationToken);
            _notifications.PublishPending();
        ...
        if (payment.Booking.MatchId is int matchId
            && !await SqlServerBookingLock.AcquireAsync(transaction, $"match-roster:{matchId}", cancellationToken))
        ...
        foreach (var item in groupPayments)
        {
            ...
            await ConfirmPaymentAsync(item, ticket, now, reference, cancellationToken);
        }
        ...
        await _paymentRepository.AddSePayTransactionAsync(NewSePayTransaction(
            request, payment.PaymentId, "Applied", utcNow), cancellationToken);
        await _paymentRepository.SaveChangesAsync(cancellationToken);
        await transaction.CommitAsync(cancellationToken);
        _notifications.PublishPending();
        PublishUpdates(groupPayments);
    ...
    private async Task NotifyAdditionalTicketTransferAsync(
        Payment payment,
    ...
    private async Task NotifyPaymentReviewRequiredAsync(
        Payment payment,
        decimal amount,
        string reference,
        string reason,
    ...
    private async Task MarkLateTicketSettlementAsync(
        Payment payment,
        SessionTicket ticket,
        DateTime now,
        string reference,
    ...
    private async Task ConfirmPaymentAsync(Payment payment, SessionTicket? ticket, DateTime now, string reference, CancellationToken cancellationToken)
    {
        var previous = payment.Status;
        payment.Status = "Paid";
        payment.PaidAt = now;
