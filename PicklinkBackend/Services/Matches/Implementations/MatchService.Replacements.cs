...
public partial class MatchService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (absence is null)
        {
            ...
            await _matchRepository.AddAbsenceAsync(absence, cancellationToken);
        }
        else
        {
            absence.Status = "Open";
        ...
        CancellationToken cancellationToken)
    {
        ...
        await using var transaction = await _matchRepository.BeginTransactionAsync(IsolationLevel.Serializable, cancellationToken);
        if (!await SqlServerBookingLock.AcquireAsync(transaction, $"match-roster:{matchId}", cancellationToken)
            || !await SqlServerBookingLock.AcquireAsync(transaction, $"player-schedule:{player.PlayerId}", cancellationToken))
        ...
        if (replacementRequest is null)
        {
            ...
            await _matchRepository.AddReplacementRequestAsync(replacementRequest, cancellationToken);
        }
        else
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        else
        {
            ...
            if (!await SqlServerBookingLock.AcquireAsync(transaction, $"player-schedule:{replacementRequest.PlayerId}", cancellationToken))
